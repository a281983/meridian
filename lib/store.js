...
async function readBlob(name) {
  ...
  // Blob has no file yet (freshly connected store). Seed it once from the
  // committed local data so the demo dashboard isn't empty the moment Blob
  // goes live; subsequent writes (new applications) append to Blob from here.
  const seed = readLocal(name);
  if (seed.length) {
    try {
      await writeBlob(name, seed);
    } catch {
      // If the seeding write fails, still return the seed for this render.
    }
  }
  return seed;
}
